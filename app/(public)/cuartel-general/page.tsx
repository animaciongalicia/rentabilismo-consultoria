...
export const metadata = {
  ...
  description: "Tu arsenal de agentes de IA y herramientas gratuitas para trabajar tu negocio. Sin rodeos, sin contenido genérico.",
};

export default async function CuartelGeneralPublicPage() {
  // Intentar obtener el usuario (puede ser null si no está logueado)
  ...
  if (user) {
    const { data: profile } = await supabase
      ...
      .eq("id", user.id)
      .single();
    hasPaid = hasFullAccess(profile?.has_paid ?? false, profile?.role, profile?.plan);
  }
          ...
          <div style={{
            ...
            <p style={{ fontSize: "0.875rem", margin: 0 }}>
              {user
                ...
                : "Los agentes de IA están incluidos en el programa. Únete para usarlos."}
            </p>
            <Link
              href={user ? "/programa" : "/registro"}
